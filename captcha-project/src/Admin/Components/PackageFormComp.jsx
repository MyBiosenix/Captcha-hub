...
function PackageFormComp() {
  ...
  const handlePackageSubmit = async (e) => {
    ...
    try {
      if (packageToEdit) {
        await axios.put(`https://api.captcha-google.com/api/types/edit-package/${packageToEdit._id}`, { packages, price });
        alert('Package Type Updated Successfully');
      } else {
        await axios.post('https://api.captcha-google.com/api/types/package-type', { packages, price });
